function F=QML_RANSAC(Sig,a,b,WL,num,NS,T,number_samples,ord)
Siggg=Sig;
for iii=1:num
...
%figure; imagesc(I)
    ...
    for i1=1:NS
        ...
        %imagesc(I)
        ind=find(IF>0);
        IF=IF_param(IF(:),ord,ind);
        %IF=interp1(ind,IF(ind),1:length(Sig),'pcubic','extrap');
        %figure; plot(IF)
        IF=round(IF);
        IF(IF<1)=1;
     ...
     %m(i1)=1*sum(IA)+1*sum(abs(diff(IO(2:8:end))))+1*sum(abs(diff(IF(1:8:end))))+0*sum(abs(diff(IF,2)));
       ...
       % m(i1)=abs(sum(Sig.*s_dechirp));
        m(i1)=abs(sum(Siggg.*s_dechirp));
